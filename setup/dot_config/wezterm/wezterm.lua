...
local function center(text, width)
    local tab_text_length = wezterm.column_width(text)

    if tab_text_length >= width then
        return text
    end

    local total_tab_width = width - tab_text_length
    local left_padding = math.floor(total_tab_width / 2)

    return string.rep(' ', left_padding) .. text .. string.rep(' ', total_tab_width - left_padding)
end

wezterm.on('format-tab-title', function(tab, tabs, panes, config, hover, max_width)
    local title = tab.active_pane.title

    title = center(title, TAB_MIN_WIDTH)
